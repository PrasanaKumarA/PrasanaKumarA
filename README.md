<div align="center">

<!-- ═══════════════════════════════════════════════ -->
<!-- ██  RETRO PLATFORMER HERO ANIMATION           ██ -->
<!-- ═══════════════════════════════════════════════ -->

<img src="https://raw.githubusercontent.com/PrasanaKumarA/PrasanaKumarA/main/assets/retro-hero-banner.svg" width="100%"/>

<!-- ═══════════════════════════════════════════════ -->
<!-- ██  ARCADE BOOT SCREEN HEADER                ██ -->
<!-- ═══════════════════════════════════════════════ -->

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0d0221,25:ff00c8,60:00f0ff,100:0d0221&height=8&width=100%"/>

<img src="https://capsule-render.vercel.app/api?type=slice&color=0:0d0221,25:ff00c8,60:7000ff,100:00f0ff&height=230&section=header&text=PRASANA%20KUMAR%20A&fontSize=46&fontColor=ffffff&fontAlignY=34&animation=twinkling&desc=LVL%2099%20%7C%20DEVOPS%20ARCHITECT%20CLASS%20%7C%20TECH4GOOD%20FELLOW&descSize=15&descColor=00f0ff&descAlignY=56"/>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:00f0ff,50:ff00c8,100:0d0221&height=8&width=100%"/>

<br/>

<!-- RETRO TYPING TERMINAL -->
<a href="https://github.com/PrasanaKumarA">
  <img src="https://readme-typing-svg.demolab.com?font=Press+Start+2P&size=13&duration=3000&pause=1000&color=00F0FF&center=true&vCenter=true&multiline=true&repeat=true&width=850&height=110&lines=%3E+LOADING+PLAYER+PROFILE...;%3E+CLASS%3A+DEVOPS+ARCHITECT;%3E+GUILD%3A+AZIM+PREMJI+FOUNDATION;%3E+PRESS+START+TO+CONTINUE" alt="Terminal Boot"/>
</a>

<br/>

<!-- HUD STATUS BAR -->
<p align="center">
  <img src="https://img.shields.io/badge/❤️_HP-100%2F100-ff2e63?style=for-the-badge&labelColor=0d0221"/>
  <img src="https://img.shields.io/badge/⚡_MP-100%2F100-00f0ff?style=for-the-badge&labelColor=0d0221"/>
  <img src="https://img.shields.io/badge/⭐_XP-99%2C999-ffd700?style=for-the-badge&labelColor=0d0221"/>
  <img src="https://img.shields.io/badge/🕹️_STATUS-ONLINE-39ff14?style=for-the-badge&labelColor=0d0221"/>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=PrasanaKumarA&style=for-the-badge&color=ff00c8&label=TIMES+SUMMONED&labelColor=0d0221"/>
  <img src="https://img.shields.io/github/followers/PrasanaKumarA?label=PARTY+MEMBERS&style=for-the-badge&logo=github&logoColor=white&color=7000ff&labelColor=0d0221"/>
</p>

</div>

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" height="4px"/>

## 🎮 &nbsp;PLAYER PROFILE

<table width="100%">
<tr>
<td width="60%" valign="top">

```
╔══════════════════════════════════════════╗
║           CHARACTER  SHEET               ║
╠══════════════════════════════════════════╣
║  NAME     : Prasana Kumar A               ║
║  CLASS    : DevOps Architect              ║
║  GUILD    : Tech4Good × Azim Premji Fdn.  ║
║  ORIGIN   : India 🇮🇳                     ║
║  RANK     : Fellow  (Aug 2025 – Present)  ║
║  ALIGN.   : Open Source / Chaotic Good    ║
╚══════════════════════════════════════════╝
```

I build and automate **cloud infrastructure**, **Kubernetes clusters**, and **observability pipelines** — grinding real-world side quests into production-grade systems. Currently on an active campaign with the **Azim Premji Foundation** as a Tech4Good Fellow, engineering open-source software for social impact.

**🎯 Main Quest:** *"Automate infrastructure, achieve 99.9% telemetry uptime, and ship open-source builds that matter."*

</td>
<td width="40%" align="center" valign="middle">

**⚔️ ATTRIBUTE POINTS**

`STR ` `Infra Ops`  ▰▰▰▰▰▰▰▰▰▱ 90
`INT ` `Backend Dev` ▰▰▰▰▰▰▰▰▱▱ 85
`DEX ` `Automation`  ▰▰▰▰▰▰▰▰▰▱ 88
`WIS ` `Observability` ▰▰▰▰▰▰▰▱▱▱ 80
`CHA ` `Collaboration` ▰▰▰▰▰▰▰▰▱▱ 82

</td>
</tr>
</table>

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" height="4px"/>

## 🌳 &nbsp;SKILL TREE

<div align="center">

<a href="https://skillicons.dev">
  <img src="https://skillicons.dev/icons?i=python,js,php,html,css,postgres,bash,docker,kubernetes,aws,nginx,linux,ubuntu,git,github,cypress&perline=8" alt="Skill Tree Grid"/>
</a>

<br/><br/>

<table>
<tr>
<td width="25%" valign="top">

**🗡️ Class: Coder**
<br/>
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat-square&logo=php&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=postgresql&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnubash&logoColor=white)

</td>
<td width="25%" valign="top">

**🛠️ Class: Builder**
<br/>
![Frappe](https://img.shields.io/badge/Frappe-0089FF?style=flat-square&logo=frappe&logoColor=white)
![ERPNext](https://img.shields.io/badge/ERPNext-5865F2?style=flat-square&logo=erpnext&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/Express-000000?style=flat-square&logo=express&logoColor=white)

</td>
<td width="25%" valign="top">

**🛡️ Class: Ops Tank**
<br/>
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat-square&logo=kubernetes&logoColor=white)
![K3s](https://img.shields.io/badge/K3s-FFC61C?style=flat-square&logo=rancher&logoColor=black)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazon-aws&logoColor=white)
![Nginx](https://img.shields.io/badge/Nginx-009639?style=flat-square&logo=nginx&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)

</td>
<td width="25%" valign="top">

**🔮 Class: Seer (QA/Monitoring)**
<br/>
![Prometheus](https://img.shields.io/badge/Prometheus-E6522C?style=flat-square&logo=prometheus&logoColor=white)
![Grafana](https://img.shields.io/badge/Grafana-F46800?style=flat-square&logo=grafana&logoColor=white)
![Cypress](https://img.shields.io/badge/Cypress-17202C?style=flat-square&logo=cypress&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)

</td>
</tr>
</table>

</div>

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" height="4px"/>

## 📜 &nbsp;ACTIVE QUEST

<table width="100%">
<tr><td>

### 🏛️ Guild: Tech4Good Community × Azim Premji Foundation
**Role:** DevOps Architect (Fellow) &nbsp;|&nbsp; **Quest Started:** August 2025 &nbsp;|&nbsp; **Status:** 🟢 In Progress

Grinding through an ongoing campaign to architect enterprise open-source software and production DevOps infrastructure:

- 🗡️ **Coding Arc:** Building scalable web platforms with Python, JavaScript, PHP, and Frappe Framework
- 🛡️ **Ops Arc:** Provisioning Docker microservices, Kubernetes clusters, and web server proxies
- 🔮 **Telemetry Arc:** Real-time Prometheus metrics, Grafana dashboards, automated WhatsApp incident alerts
- 🧪 **Trial Arc:** Cypress E2E test suites for continuous application stability
- ⚙️ **Grind Arc:** Automating Linux workflows, shell scripting, and cloud backup management

</td></tr>
</table>

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" height="4px"/>

## 🎒 &nbsp;INVENTORY — LOOT & ARTIFACTS

<table width="100%">
<tr>
<td colspan="2" align="center">

### 🟣 [LEGENDARY] Grafana + Prometheus + WhatsApp Monitoring
`grafana-prometheus-whatsapp-monitoring`

</td>
</tr>
<tr>
<td width="75%" valign="top">

**Effect:** Prevents silent server crashes — grants real-time HP (CPU/RAM) monitoring on any Linux host and instantly pings the party (on-call engineers) via WhatsApp when HP drops below threshold.

**Crafted with:** `Docker` `Prometheus` `Grafana` `Node Exporter` `Node.js` `WhatsApp API`

</td>
<td width="25%" align="center" valign="middle">

[![Loot](https://img.shields.io/badge/OPEN_CHEST-ff00c8?style=for-the-badge&logo=github&logoColor=white)](https://github.com/PrasanaKumarA/grafana-prometheus-whatsapp-monitoring)

</td>
</tr>
</table>

<br/>

<table width="100%">
<tr>
<td width="50%" valign="top">

#### 🔵 [EPIC] K3s etcd Backup & Restore
Grants disaster-recovery revival — automated snapshot backup and restore for K3s cluster state, stashed safely in AWS S3.

`Kubernetes` `K3s` `etcd` `AWS S3` `Bash`
[→ Open Chest](https://github.com/PrasanaKumarA)

</td>
<td width="50%" valign="top">

#### 🔵 [EPIC] Private Docker Registry
A self-hosted vault — secure private container image registry with custom auth and Compose orchestration.

`Docker Registry` `Docker Compose` `Linux` `Nginx`
[→ Open Chest](https://github.com/PrasanaKumarA)

</td>
</tr>
<tr>
<td width="50%" valign="top">

#### 🟢 [RARE] Whatomate
Auto-cast messaging spell — WhatsApp Business API automation engine on Node.js, Docker, PostgreSQL, and Redis.

`Node.js` `PostgreSQL` `Redis` `Docker`
[→ Open Chest](https://github.com/PrasanaKumarA/WhatoMate)

</td>
<td width="50%" valign="top">

#### 🟢 [RARE] IdliStack Cypress Testing
A scrying orb — automated E2E test suite for UI testing, bug detection, and execution reports.

`Cypress` `JavaScript` `E2E Testing` `QA`
[→ Open Chest](https://github.com/PrasanaKumarA/Idlistack-testing-Cypress-)

</td>
</tr>
<tr>
<td width="50%" valign="top">

#### ⚪ [COMMON] Arockia Electricals
A merchant's ledger — web app for enterprise inventory tracking and billing workflow management.

`PHP` `HTML/CSS` `JavaScript` `SQL`
[→ Open Chest](https://github.com/PrasanaKumarA/Arockia-Electricals)

</td>
<td width="50%" valign="top">

#### ⚪ [COMMON] Idlibook v0.0.1
A treasurer's tome — enterprise financial accounting app replicating core ledger management features.

`Python` `ERPNext` `Frappe Framework` `SQL`
[→ Open Chest](https://github.com/PrasanaKumarA/Idlibook-v.0.0.1)

</td>
</tr>
</table>

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" height="4px"/>

## 🗺️ &nbsp;DUNGEON MAP — SYSTEM ARCHITECTURE

**Boss Fight: Prometheus + Grafana + WhatsApp Telemetry Flow**

```text
  ┌──────────────────────┐
  │   Linux Host OS      │   (Level 1 — Spawn Point)
  │  (Node Exporter)     │
  └──────────┬───────────┘
             │ scrape /metrics every 15s
             ▼
  ┌──────────────────────┐
  │  Prometheus Server   │   (Level 2 — Watchtower)
  └──────────┬───────────┘
             │ evaluate alert rules
             ▼
  ┌──────────────────────┐   trigger webhook   ┌────────────────────────┐
  │  Grafana Dashboard   │ ───────────────────> │  Node.js Webhook Server│  (Level 3 — Messenger)
  └──────────────────────┘                      └───────────┬────────────┘
                                                             │ cast spell
                                                             ▼
                                                 ┌────────────────────────┐
                                                 │  WhatsApp Cloud API    │  (Final Boss — Alert Delivered)
                                                 └────────────────────────┘
```

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" height="4px"/>

## 🏆 &nbsp;ACHIEVEMENTS UNLOCKED

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=PrasanaKumarA&theme=radical&column=6&margin-w=12&margin-h=12&no-bg=true" width="100%"/>

</div>

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" height="4px"/>

## 📊 &nbsp;LEADERBOARD — GAME STATS

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=PrasanaKumarA&show_icons=true&theme=radical&hide_border=true&bg_color=0d0221&title_color=ff00c8&icon_color=00f0ff&text_color=c9d1d9&border_radius=12" width="49%"/>
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=PrasanaKumarA&layout=compact&theme=radical&hide_border=true&bg_color=0d0221&title_color=ff00c8&text_color=c9d1d9&border_radius=12" width="49%"/>

<img src="https://streak-stats.demolab.com/?user=PrasanaKumarA&theme=dark&hide_border=true&ring=ff00c8&fire=00f0ff&currStreakLabel=00f0ff&background=0d0221" width="100%"/>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=PrasanaKumarA&bg_color=0d0221&color=00f0ff&line=ff00c8&point=ffffff&area=true&area_color=7000ff&hide_border=true&custom_title=DAILY+GRIND+LOG" width="100%"/>

</div>

> 🕹️ **Want a real moving animation, not just a static bar?** I can add a commit-eating **snake game** (`platane/snk` GitHub Action) that visually plays across your contribution graph in neon colors — genuinely animated, updates daily via a scheduled workflow. Say the word and I'll write the `.yml` for you.

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" height="4px"/>

## 🕹️ &nbsp;CO-OP INVITE

<div align="center">

**Press START to join the party** — open to collaborating on DevOps, cloud infra, and open-source builds.

[![GitHub](https://img.shields.io/badge/GitHub-PrasanaKumarA-ff00c8?style=for-the-badge&logo=github&logoColor=white)](https://github.com/PrasanaKumarA)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-00f0ff?style=for-the-badge&logo=linkedin&logoColor=black)](https://linkedin.com/in/PrasanaKumarA)
[![Email](https://img.shields.io/badge/Email-Contact_Me-7000ff?style=for-the-badge&logo=gmail&logoColor=white)](mailto:prasana.kumar.a@example.com)

</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:00f0ff,50:ff00c8,100:0d0221&height=8&width=100%"/>

<img src="https://capsule-render.vercel.app/api?type=slice&color=0:0d0221,25:7000ff,60:ff00c8,100:0d0221&height=110&section=footer"/>

<div align="center">

`🕹️ GAME OVER? — INSERT COMMIT TO CONTINUE ▮▮▮ PRESS START`

</div>